...
class promp_pub(object):
	...
	def __init__(self):
		...
		self.q0 = [0.0, (-60*np.pi)/180, 0.0, (120*np.pi)/180, 0.0, (-np.pi)/3, 0.0] ## works for end effector facing down and cluster right above franka head
	...
	def promp_generator(self, IC, goal):
		...
		basisGenerator1 = basis.NormalizedRBFBasisGenerator(phaseGenerator, numBasis=5, duration=self.Tf, basisBandWidthFactor=2, # check duration
		                                                   numBasisOutside=3) 
