...
class takayuki_ral:
    ...
    def get_par_promp(self, basisgenerator):
        ...
        for i in range(len(self.ee_trj)):
            ...
            # plt.figure(1)
            # # plt.plot(x0[0][:, 0], 'r', x0[0][:, 1], 'b', x0[0][:, 2], 'g')
            # plt.plot(x0[0][:, 0], 'r')
            # plt.figure(2)
            # plt.plot(x0[0][:, 1], 'b')
            # plt.figure(3)
        ...
        xMEAN = xMean[0, :, :3]
        # plt.figure(5)
        # plt.plot(xCov[:, 0, 0], 'r', xCov[:, 1, 1], 'b', xCov[:, 2, 2], 'g')
        #plt.show()

        w1 = np.reshape(w1_all, [len(self.ee_trj), basisgenerator.numBasis])
        w2 = np.reshape(w2_all, [len(self.ee_trj), basisgenerator.numBasis])
        w3 = np.reshape(w3_all, [len(self.ee_trj), basisgenerator.numBasis])
        wm1 = np.mean(w1, axis=0)
        wm2 = np.mean(w2, axis=0)
        wm3 = np.mean(w3, axis=0)
        x1 = np.array([np.inner(basisfunc, wm1)])
        x2 = np.array([np.inner(basisfunc, wm2)])
        x3 = np.array([np.inner(basisfunc, wm3)])
...
class trajectory_optimization():
    ...
    def calculate_t_cost_ral(self, trajectory):
        ...
        return 10 * F_smooth + 1.5 * obstacle_cost + self.w_dem * dem_cost
